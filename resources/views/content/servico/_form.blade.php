...
<div class="input-field">
	<input type="text" name="descricao" required class="validade" value="{{ isset($servico->descricao) ? $servico->descricao : '' }}">
...
<div class="input-field">
	<select name="idSegmento" class="validade">
		...
		<option value="{{ $Segmento->idSegmento }}" {{ isset($servico->idSegmento) && $servico->idSegmento == $Segmento->idSegmento ? 'selected' : '' }}>{{ $Segmento->segmento }}</option>
		@endforeach
	</select>
